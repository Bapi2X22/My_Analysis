...
def main():
    parser = argparse.ArgumentParser(description="Plot Jet fields from Coffea NanoEvents")
    ...
    # Default list
    plot_list = [
        {"var": "pt", "bins": (100, 0, 200), "title": "Jet p_{T} [GeV]", "legend": "Jet pT"},
        {"var": "eta", "bins": (60, -3, 3), "title": "Jet #eta", "legend": "Jet eta"},
        {"var": "phi", "bins": (64, -3.2, 3.2), "title": "Jet #phi", "legend": "Jet phi"},
    ]
    ...
    # Extract jets
    jets = events.Jet
    print(f"Loaded {len(events)} events, {ak.num(jets)} jets/event (avg {ak.mean(ak.num(jets)):.2f})")

    # Prepare output ROOT
    if not os.path.exists(args.outdir):
        os.makedirs(args.outdir)
    outroot_path = os.path.join(args.outdir, "jet_plots.root")
    outroot = ROOT.TFile(outroot_path, "RECREATE")

    n_jets = ak.num(jets)
    print("Plotting number of jets per event (default) ...")
    plot_histogram(
        "nJets",
        n_jets,
        (10, 0, 10),
    ...
    for cfg in plot_list:
        ...
        try:
            values = getattr(jets, varname)
        except AttributeError:
            print(f"Warning: Variable '{varname}' not found in Jet branch. Skipping.")
            continue
