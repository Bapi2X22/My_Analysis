...
out_dir = "/eos/user/b/bbapi/www/Systematics_study/Old_presel"
...
nominal_jet_dir    = base_dir + "nominal/dijet/"
...
def load_common_triplet(file_nom, up_dir, down_dir):
    ...
    idx_nom  = ak.to_numpy(file_nom.event_idx)
    idx_up   = ak.to_numpy(file_up.event_idx)
    idx_down = ak.to_numpy(file_down.event_idx)
...
jet_jec_branches = {
    "lead": {
        "nom":  "bJetlead_pt_jec",
        "up":   "bJetlead_pt_jec_syst_Total_up",
        "down": "bJetlead_pt_jec_syst_Total_down",
    },  
    "sublead": {
        "nom":  "bJetsublead_pt_jec",
        "up":   "bJetsublead_pt_jec_syst_Total_up",
        "down": "bJetsublead_pt_jec_syst_Total_down",
    },
}
...
for syst, pair in syst_pairs.items():
    ...
    for tag in ["lead", "sublead"]:
        ...
        if is_jec:
            ...
            pt_up   = np.asarray(getattr(file_nom, jet_jec_branches[tag]["up"]))
            pt_down = np.asarray(getattr(file_nom, jet_jec_branches[tag]["down"]))
            obj_label = "jet"
        ...
        latex.DrawLatex(0.58, 0.85, f"m_{{A}} = {M} GeV")
        ...
        if DO_MASS and (not is_jec):
            ...
            latex_m.DrawLatex(0.58, 0.85, f"m_{{A}} = {M} GeV")
        ...
        latex_r.DrawLatex(0.58, 0.85, f"m_{{A}} = {M} GeV")
