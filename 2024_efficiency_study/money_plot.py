...
def get_category_fractions(file, mass):
    ...
    # )) & (fj.btagUParTAK4probbb[:, 0] > diB_mask_thr)
    ...
    frac_res = ak.sum(resolved) / total
    frac_mer = ak.sum(merged) / total
    frac_one = ak.sum(one_jet) / total

    return frac_res, frac_mer, frac_one
...
for M in mass_points:
    ...
    f_res, f_mer, f_one = get_category_fractions(file, M)

    fractions_res.append(f_res)
    fractions_mer.append(f_mer)
    fractions_one.append(f_one)

n = len(mass_points)

h_res = ROOT.TH1F("h_res", "", n, 0, n)
h_mer = ROOT.TH1F("h_mer", "", n, 0, n)
h_one = ROOT.TH1F("h_one", "", n, 0, n)

for i, m in enumerate(mass_points):
    bin_idx = i + 1

    h_res.SetBinContent(bin_idx, fractions_res[i])
    h_mer.SetBinContent(bin_idx, fractions_mer[i])
    h_one.SetBinContent(bin_idx, fractions_one[i])

    h_res.GetXaxis().SetBinLabel(bin_idx, f"{m}")
    h_mer.GetXaxis().SetBinLabel(bin_idx, f"{m}")
    h_one.GetXaxis().SetBinLabel(bin_idx, f"{m}")


h_res.SetFillColor(ROOT.kBlue-7)
h_mer.SetFillColor(ROOT.kRed-4)
h_one.SetFillColor(ROOT.kGreen+2)

h_res.SetLineColor(ROOT.kBlue-7)
h_mer.SetLineColor(ROOT.kRed-4)
h_one.SetLineColor(ROOT.kGreen+2)

for h in [h_res, h_mer, h_one]:
...
c = ROOT.TCanvas("c", "Category Fractions", 800, 600)

stack.Draw("HIST")

stack.GetYaxis().SetTitle("Fraction of Events")
stack.GetXaxis().SetTitle("m_{A} [GeV]")
...
leg = ROOT.TLegend(0.50, 0.65, 0.85, 0.88)
leg.SetBorderSize(0)
leg.SetFillStyle(0)
leg.SetTextSize(0.025)

leg.AddEntry(h_res, "Resolved (#geq 2 jets)", "f")
# leg.AddEntry(h_mer, "Merged (probbb > probb) & probbb > " + str(diB_mask_thr), "f")
# leg.AddEntry(h_one, "One-jet (probb > probbb) | probbb < " + str(diB_mask_thr), "f")
leg.AddEntry(h_mer, "Merged (probbb > probb)", "f")
leg.AddEntry(h_one, "One-jet (probb > probbb)", "f")

leg.Draw()


c.Update()
c.SaveAs(Plot_dir+"CategoryFractionMoneyPlot_nothr.pdf")
c.SaveAs(Plot_dir+"CategoryFractionMoneyPlot_nothr.png")
