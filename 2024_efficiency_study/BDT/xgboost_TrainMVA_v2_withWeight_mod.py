...
def loadMCBackground(inputVars, inputVarsMC):
  ...
  # dataset_Bkg = uproot.open('merged_bkg.root')['DiphotonTree'].arrays(inputVars,library='pd')
  dataset_Bkg = uproot.open('merged_bkg.root')['DiphotonTree'].arrays(inputVars)
...
if __name__ == '__main__':
  ...
  if useMC:
    ...
    df_bkg = df_bkg.sample(frac=1/5.0, random_state=12345)
...
#   print(bkg_sum)
  ...
  # X = pd.concat((df_sig.iloc[:,0:21] ,df_bkg.iloc[:,0:21] ))
  features = ['evt_wgt'] + inputVars
...
#  test_size = 0.001
  ...
  W_train = X_train['evt_wgt'].values *(10**5)
  X_train = X_train.drop('evt_wgt',axis=1).values
  Y_train = Y_train.values
  W_test  = X_test['evt_wgt'].values *(10**5)
  ...
  drawScores(predY_train,predY_test,W_train,W_test,25,0.,1.,'BDT'+label)
